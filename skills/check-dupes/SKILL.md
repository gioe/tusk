...
**Callers:** `/retro` runs `check` explicitly before each insert; `/create-task` and `/review-commits` rely on `tusk task-insert` which runs the check internally; `/groom-backlog` runs `scan` during its duplicate sweep.
