...
After TASK-317, the per-branch classification logic (effective first-token
resolution, /-path short-circuit per Issue #589, on-PATH check, post-sandbox
malformed/environmental/interpreter-wrapper-bypass routing) lives in
`bin/tusk-address-issue.py` and is exercised by `tests/unit/test_classify_spec.py`.
SKILL.md prose tests here are reduced to the surface-level guarantees that
remain in the collapsed Step 4.1 (it still mentions wrapper handling at a
conceptual level and delegates to the helper).
"""
...
class TestAddressIssueStep41WrapperHandling:
    ...
    def test_skill_delegates_classification_to_helper(self):
        """Step 4.1 must call into `tusk address-issue classify-spec` rather
        than reimplementing the per-branch decision tree as inline prose. The
        helper owns wrapper peel, /-path short-circuit (Issue #589), on-PATH
        check, and the post-sandbox malformed/environmental/interpreter-
        wrapper-bypass routing — Step 4.1 is reduced to extract → call helper
        → act on the returned tuple. Per-branch guarantees live in
        tests/unit/test_classify_spec.py."""
        text = _skill_text()
        assert "tusk address-issue classify-spec" in text, (
            "Step 4.1 must delegate classification to the classify-spec helper"
        )
