...
def make_root(files: dict[str, str]) -> str:
    """Create a temp directory tree from {rel_path: content} and return its path."""
...
class TestMarkdownSkipped:
    ...
    def test_any_md_in_skills_dir_no_violation(self):
        ...
        root = make_root(
            {"skills/foo/FOCUS.md": "AND foo != 'bar'\n"}
        )
        assert lint.rule2_sql_not_equal(root) == []
