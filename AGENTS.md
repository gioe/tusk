...
bin/tusk task-import --file tasks.json [--dry-run] [--best-effort]   # bulk-create tasks from {"tasks":[...]} JSON; atomic by default, --best-effort commits valid rows while reporting per-task failures
printf '%s\n' '{"tasks":[{"key":"docs","summary":"Document import","description":"Write command docs","domain":"docs","task_type":"docs","criteria":["Docs cover file and stdin usage"]}]}' | bin/tusk task-import --stdin --dry-run
# Prefer task-import over repeated task-insert calls when materializing multiple approved tasks: it validates the whole batch first, supports local dependency keys via depends_on, links objectives, centralizes duplicate policy (fail|skip|allow), and returns one created/skipped/failed JSON result.
...
bin/tusk init-intent normalize --answers <json>  # → {"success": true, "intent": {...}} normalized project intent for tusk-init
bin/tusk init-write-config [--domains <json>] [--agents <json>] [--task-types <json>] [--test-command <cmd>] [--project-type <type>] [--init-intent <json>] [--project-libs <json>] [--worktree-symlink-files <json>]  # → {"success": bool, "config_path": "...", "backed_up": bool}
...
Additional subcommands (`blockers`, `review`, `chain`, `loop`, `deps blocked/all`, `session-stats`, `session-close`, `session-recalc`, `skill-run`, `call-breakdown`, `token-audit`, `pricing-update`, `sync-skills`, `dashboard`) follow the same `bin/tusk <cmd> --help` pattern. `bin/tusk help <cmd>` is an equivalent safe discovery entry point.
...
- **`retry_on_locked` / `run_write` retry the whole unit of work with bounded exponential backoff** for the residual upgrade/open-cursor cases that even IMMEDIATE can't avoid — each attempt opens a fresh connection so the SHARED lock is dropped between tries. Tunable via env `TUSK_WRITE_RETRIES` (default 6 retries after the first try) and `TUSK_WRITE_RETRY_BASE_MS` (default 25ms, doubling per attempt, capped at 2s). On budget exhaustion a one-line actionable stderr diagnostic is emitted (not a raw traceback) and the original `OperationalError` is re-raised. High-frequency write paths — `skill-run` finish/start/cancel, `criteria`, `review`, `progress`, `context`, `jot` — are routed through this layer; raise the knobs under unusually heavy parallel load.
- **The database runs in WAL journal mode (issue #1144, migration 84) — the deeper mitigation.** In WAL, readers use snapshots and never block writers, so the reader→writer SHARED→RESERVED upgrade deadlock above does not arise in the first place; only writer-vs-writer contention remains, which `busy_timeout` waits on. `cmd_init` enables WAL for fresh installs and migration 84 switches pre-existing `delete`-mode DBs over. The two layers above remain as defense-in-depth. See `docs/DOMAIN.md` → "Database Scope and Portability".
...
- **`/objective`** — Plan an existing objective without execution (`plan OBJ-N`), run an already-planned objective (`run OBJ-N`), or retain the complete create-plan-run-close lifecycle (`full <intent>`; bare intent remains compatible)
...
- **Claude Code project** (`.claude/` present): copies `bin/tusk` + `bin/tusk-*.py` + `VERSION` + `config.default.json` + `pricing.json` → `.claude/bin/`, skills → `.claude/skills/`, hooks → `.claude/hooks/`, merges `.claude/settings.json`, runs `tusk init` + `tusk migrate`.
