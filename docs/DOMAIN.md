...
|------|---------|---------|
| `task_metrics` | Aggregates session cost/tokens/lines/request_count per task (exposes `total_request_count` = SUM of `task_sessions.request_count`, plus `reopen_count` = count of `task_status_transitions` rows whose `to_status` is `'To Do'` — a forward-looking rework signal capturing both `In Progress → To Do` mid-task rework and `Done → To Do` post-Done reopens via `tusk task-reopen --force`; always `0` for tasks that were already closed before migration 53, since migration 53's backfill produces no `to_status='To Do'` rows). Projects `t.*` from `tasks`, so its column list is frozen at CREATE VIEW time and was recreated in migration 56 to pick up `fixes_task_id` (and any subsequent ALTER-added `tasks` columns) on DBs that migrated through v55. | `tusk-dashboard.py`, reporting |
| `v_ready_tasks` | Canonical "ready to work" definition: To Do, all `blocks`-type deps Done, no open external blockers (contingent deps do not prevent readiness). Projects `t.*`; recreated in migration 56 alongside `task_metrics` to propagate ALTER-added `tasks` columns. | `/tusk`, `tusk-loop.py`, `tusk deps ready` |
| `v_chain_heads` | Non-Done tasks with unfinished downstream dependents and no unmet upstream deps. Projects `t.*`; recreated in migration 56. | `/chain` |
...
| `v_criteria_coverage` | Per-task counts of total, completed, and remaining criteria (deferred excluded). Projects specific `tasks` columns (not `t.*`), so ALTER additions do not silently drop out of its projection, but it was recreated in migration 56 to keep the set of tasks-dependent views uniform. | Reporting, `/tusk-insights` |
