...
| `not_before` | TEXT | nullable | UTC datetime; task is hidden from ready-task selection while this timestamp is in the future. Set by `tusk task-insert --not-before <timestamp|+offset>`; direct `tusk task-start <id>` refuses until the timestamp passes unless `--force-not-before` is supplied. Added in migration 75 |
...
| `merge_commit_sha` | TEXT | nullable | The commit SHA of the merge that closed the task. Stamped by `tusk merge` for all three merge paths (default-branch ff, no-checkout fast-forward push, PR squash via `gh pr view --json mergeCommit`). Consumed by `tusk task-summary`'s `fetch_diff` as a deterministic fast-path (`git show --first-parent --numstat <sha>`) for tasks that have not been reopened. A later merge overwrites this stamp, so tasks with a recorded transition back to `To Do` bypass the fast path and scan every task-prefixed commit to recover cumulative lifecycle totals without including unrelated intervening commits. Stays NULL for `tusk abandon`-closed tasks and for legacy tasks closed before migration 70 — those fall through to the existing recovery chain. Best-effort write at merge close; never aborts the merge. Added in migration 70 |
...
**Progressive spec rigor:** task pickup should scale handoff expectations with `tasks.complexity`. `XS` and `S` tasks only need clear criteria; adding objective or context ceremony would usually cost more than it saves. `M` tasks should avoid vague, non-observable acceptance criteria and include at least one verification-backed criterion when the proof can be executed or inspected. `L` and `XL` tasks should usually also be linked to an objective and carry active `risk`, `assumption`, or `decision` context when those signals exist. `tusk task-start` treats these checks as advisory warnings, not hard blockers, so agents can preserve momentum while still seeing weak specs before implementation begins.
...
- `operator_declared` — set via `tusk task-insert --scope <pattern>` (repeatable) or by implicit `tusk scope add` before task start. The operator named these paths up-front. The CLI accepts an explicit source as an administrative compatibility override, but normal task workflows do not use it after task start.
...
2. Before task start, implicit `tusk scope add ... --reason "..."` records additional up-front declarations as `operator_declared`. Scope omitted before this boundary cannot be retroactively categorized as up-front by a normal task workflow.
3. After task start, implicit `tusk scope add ... --reason "..."` records every addition as an `expanded_mid_task` row, even before edits, progress checkpoints, criteria completions, or commits.
...
| `diff_range` | TEXT | nullable | Git diff range resolved by `tusk review begin` and stamped on insert (e.g. `origin/main...HEAD`, `<sha>^..<sha>`). Reused by `tusk review validate-comments` so the validator runs `git diff --name-only` against the same range used to record findings — without persistence the validator would re-derive via `compute_range` and drift when new commits land between begin and validate (issue #847). Added in migration 69; historical rows stay NULL and the validator falls back to recomputation for them. |
| `cost_dollars` | REAL | nullable | AI cost of this review pass. Auto-populated by `tusk review approve` / `request-changes` from the transcript window `[code_reviews.created_at, now]` (same pricing-lib helpers as `tusk skill-run finish`). Override with `--cost-dollars`; opt out with `--skip-cost`. Recover a row that finalized without cost data via `tusk review backfill-cost <review_id>` (best-effort: only works while the row's transcript window is still on disk). NULL when no transcript is discoverable for the window — historical pre-v801 rows stay NULL because their transcripts may no longer exist on disk. |
...
| `uncached_tokens_in` | INTEGER | nullable | Non-cached input tokens — priced as regular input. Populated from `totals[input_tokens]`. Added in migration 74. |
| `model` | TEXT | nullable | Dominant model used during the run. `tusk skill-run finish` writes `(unknown)` when a transcript exists but the run window has no attributable model/request, and `(transcript missing)` when no Claude JSONL transcript is discoverable at finish time; cancelled runs keep an empty model string so `tusk skill-run list` can label them separately. Dashboard queries that group by model should preserve `(transcript missing)` as its own bucket rather than folding it into `unknown`. |
...
| `v_ready_tasks` | Canonical "ready to work" definition: To Do, all `blocks`-type deps Done, no open external blockers (contingent deps do not prevent readiness), no future `not_before` gate, and not a bake-off shadow. Projects `t.*`; recreated in migration 56 alongside `task_metrics`, in migration 58 to add the `bakeoff_shadow = 0` filter, and in migration 75 to add the `not_before` readiness filter. | `/tusk`, `tusk-loop.py`, `tusk deps ready` |
| `v_chain_heads` | Non-Done tasks with unfinished downstream dependents and no unmet upstream deps, no open external blockers, no future `not_before` gate, and excluding bake-off shadows. Projects `t.*`; recreated in migration 56, in migration 58 for the shadow filter, and in migration 75 for the time-gate filter. | `/chain` |
...
| `project_type` | `string \| null` | `null` | A coarse project category (e.g. `ios_app`, `python_service`) used as one signal for bootstrap pack selection. |
...
**Bootstrap pack selection:** `project_libs` is populated by `tusk init-bootstrap-select` (also called by `tusk init-write-config` when `--project-libs` is not explicit). Selection uses `project_type`, normalized `init_intent`, inferred archetype, launch platforms, and feature preferences. Built-in backward-compatible packs are:

| Pack | Repo | Signals |
|------|------|---------|
| `ios_app` | `gioe/ios-libs` | `project_type=ios_app`, `archetype=consumer_ios_app`, `platform=ios`, SwiftUI/mobile features |
| `python_service` | `gioe/python-libs` | `project_type=python_service`, `archetype=api_service`, `platform=backend`, API/Python/observability features |
