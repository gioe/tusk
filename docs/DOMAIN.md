...
**Progressive spec rigor:** task pickup should scale handoff expectations with `tasks.complexity`. `XS` and `S` tasks only need clear criteria; adding objective or context ceremony would usually cost more than it saves. `M` tasks should include at least one verification-backed criterion when the proof can be executed or inspected. `L` and `XL` tasks should usually also be linked to an objective and carry active `risk`, `assumption`, or `decision` context when those signals exist. `tusk task-start` treats these checks as advisory warnings, not hard blockers, so agents can preserve momentum while still seeing weak specs before implementation begins.
