...
| `merge_commit_sha` | TEXT | nullable | The commit SHA of the merge that closed the task. Stamped by `tusk merge` for all three merge paths (default-branch ff, no-checkout fast-forward push, PR squash via `gh pr view --json mergeCommit`). Consumed by `tusk task-summary`'s `fetch_diff` as a deterministic fast-path (`git show --first-parent --numstat <sha>`) so the ref/criterion-hash/fsck recovery chain is unneeded for tasks closed via merge. Stays NULL for `tusk abandon`-closed tasks and for legacy tasks closed before migration 70 — those fall through to the existing recovery chain. Best-effort write at merge close; never aborts the merge. Added in migration 70 |
