...
- [TASK-849] Fix task-brief cwd tracking across pipeline and background boundaries

## [1243] - 2026-07-30

- [TASK-848] Detect undistributed fixes in address-issue resolution checks

## [1242] - 2026-07-29

- [TASK-847] Fix: Make null Codex usage finalization atomic

## [1241] - 2026-07-21

- [TASK-831] Optimize task-summary baseline cost aggregation
...
- [TASK-827] Add gpt-5.6-sol pricing support
...
- [TASK-808] Show Git commit stdout when stderr is empty

## [1228] - 2026-07-17

- [TASK-806] Add a cost skill for task, objective, and project rollups
...
- [TASK-782] Fix: quarantine vendored Tusk framework tests in consumer test gates
...
- [TASK-645] Warn at merge time when main-side commits touch branch-modified files

## [1142] - 2026-06-12

- [TASK-644] Record post-merge verification outcomes: criteria done clears deferral on Done tasks
...
- [TASK-502] Add stale-primary advisory to tusk task-worktree create
...
- [TASK-494] Fix: move sparse-checkout refusal into build_manifest() so all callers are protected

## [1012] - 2026-05-26

- [TASK-480] Sparse-checkout overhaul: fix MANIFEST + Rule 18 + cone defaults
...
- [TASK-482] tusk scope CLI hardening: validate patterns + mark list readonly
...
- [TASK-471] Make task scope a first-class declared concept with lifecycle, schema, and operator API
...
- [TASK-450] Fix: tusk merge no-checkout path leaves .claude/bin/ stale (auto-refresh sees zero drift)
...
- [TASK-257] Ship /ios-libs-contribute skill
...
- [TASK-338] Auto-recover from Rule 18 (MANIFEST drift) by running tusk generate-manifest and retrying
...
- [TASK-247] Run tusk regen-triggers automatically from tusk migrate

## [790] - 2026-04-28

- [TASK-246] Add trigger-drift detection to tusk validate
...
- [TASK-236] Validate dupes.criterion_check_threshold value range in config validator
...
- [TASK-222] Fix: tusk init-wizard wipes existing task DB on populated projects

## [770] - 2026-04-27

- [TASK-221] Add --scaffold-spec / --no-scaffold flags to tusk init-wizard

## [769] - 2026-04-27

- [TASK-220] Fix: tusk upgrade should prune files now in bin/dist-excluded.txt

## [768] - 2026-04-27

- [TASK-219] Document zsh/bash backtick and $ hazard in tusk commit message guidance
...
- [TASK-180] Fix: tusk merge prints misleading 'already on main — diverged' when feature branch has no new commits
...
- [TASK-171] Extend encoding='utf-8' to subprocess.check_output / Popen and broaden Convention 25
...
- [TASK-169] Fix conventions-preflight.sh TUSK PATH preference (mirror TASK-166 fix)
...
- [TASK-147] Filter deferred tasks (`is_deferred = 1`) out of `v_ready_tasks` and `v_chain_heads` so `tusk task-start` and `/chain` never hand back a row that is, by definition, "don't pick this up yet." Both view definitions in `bin/tusk` cmd_init and the new `migrate_59` in `bin/tusk-migrate.py` add `(t.is_deferred = 0 OR t.is_deferred IS NULL)` — the `IS NULL` branch is defensive for any past schemas that bypassed the `NOT NULL DEFAULT 0` column. `task_metrics`, `v_blocked_tasks`, and `v_criteria_coverage` intentionally still surface deferred rows so cost/blocker/coverage reporting stays complete. `docs/DOMAIN.md` documents the new filter on the `is_deferred` column row and in both view descriptions. `tests/integration/test_migrate_59.py` pins the post-migration view shape against a frozen v59 snapshot and verifies the before/after behaviour — the pre-migrate assertion confirms the fixture actually reproduces the leak before migration runs, so the post-migrate assertion is not vacuous.

## [696] - 2026-04-24

- [TASK-144] Warn in `tusk task-start` when the latest `task_progress.next_steps` describes work unrelated to the current task summary/description — surfaces scope rewrites that would otherwise render stale handoff notes as authoritative at the start of the next session. `bin/tusk-task-start.py` grows `_extract_content_tokens()` (stopword-filtered, crude-stemmed) and `_progress_next_steps_is_stale()` / `_find_stale_progress_row()` helpers; if the newest non-empty `next_steps` shares fewer than 15% of its content tokens with the current summary+description, a stderr warning fires and includes the entry's `created_at` so the operator can judge how stale the handoff is. The threshold and stemming keep typo fixes, reword, and same-scope in-progress checkpoints silent; full scope rewrites trip it. `tests/integration/test_task_start_stale_progress.py` covers the quiet path (small edit, same-scope checkpoint, no progress, newer entry supersedes older), the warn path (scope rewrite includes the created_at timestamp), and helper-level guards (short next_steps, empty current text, stem collapsing for caching/cache, typo/typos, implement*).

## [695] - 2026-04-24

- [TASK-136] Add Codex-project compatibility to `install.sh`, `bin/tusk`, and `bin/tusk-upgrade.py`. `install.sh` now auto-detects the host agent layout: `.claude/` present → Claude mode (unchanged), `AGENTS.md` present with no `.claude/` → Codex mode (binaries to `tusk/bin/`, no skills/hooks/settings.json merge, manifest at `tusk/tusk-manifest.json`), neither present → hard error with a helpful message. An `install-mode` marker is stamped in the install dir so `bin/tusk` and `bin/tusk-upgrade.py` can route gitignore, agent-doc (`CLAUDE.md` vs `AGENTS.md`), and upgrade-step decisions without re-probing the project layout. `bin/tusk-upgrade.py` adds `detect_install_mode()` and `translate_manifest_for_mode()` so Codex installs can reuse the claude-shaped tarball `MANIFEST` — orphan detection translates `.claude/bin/` → `tusk/bin/` and drops `.claude/skills/`/`.claude/hooks/` entries before comparing. `docs/CODEX.md` and the `CLAUDE.md` Installation Model section document the Claude-vs-Codex matrix and the PATH-setup guidance for Codex users. `tests/integration/test_codex_install.py` (8 tests) verifies end-to-end Codex install: binaries land in `tusk/bin/`, marker is stamped, skills/hooks/settings are skipped, `AGENTS.md` gets the task-tool sentinel, re-running is idempotent, and `.gitignore` references `tusk/bin/` instead of `.claude/bin/`. `tests/unit/test_upgrade_codex_mode.py` (9 tests) covers the new helpers in isolation.
...
- [TASK-127] Fix bakeoff `pick`/`discard` leaving orphaned child rows when deleting shadow tasks. `_delete_shadow_rows` in `bin/tusk-bakeoff.py` previously only deleted `acceptance_criteria` + `tasks` rows, trusting `ON DELETE CASCADE` declarations to clean the rest — but SQLite's `PRAGMA foreign_keys` is a per-connection pragma and is OFF by default across tusk's DB, so other readers (aggregate `skill_runs` cost reports, raw `skill-run` lookups, `tool_call_events` scans) still saw rows referencing deleted shadow ids. The helper now explicitly sweeps the full child graph for each deleted shadow: `task_sessions`, `task_progress`, `skill_runs` (hard-deleted, not `SET NULL`), `code_reviews` + their `review_comments` (both the `review_id` path and `review_comments.deferred_task_id` back-references from unrelated reviews), and `tool_call_stats` + `tool_call_events` (routed through `task_id`, `session_id`, and `skill_run_id` pivots so rows linked only via intermediates are cleaned too). A new `TestDeleteShadowRowsChildCleanup` in `tests/integration/test_bakeoff.py` seeds every affected table against a shadow (plus a `deferred_task_id` back-reference from a review on the source task), calls `_delete_shadow_rows` directly, and asserts each child row is gone while the sibling shadow and source task stay untouched.

## [684] - 2026-04-20

- [TASK-129] Fix `tusk review-final-summary` reporting `0 tasks created` for deferred findings even when `tusk review-defer` actually inserted new tasks. `tusk review resolve` gains an optional `--deferred-task-id` flag that writes the created task id onto `review_comments.deferred_task_id` in the same UPDATE as `resolution`; `tusk review-defer` now threads the id returned by `tusk task-insert` through to that flag after a successful insert. Before this change review-defer only called `tusk review resolve <id> deferred`, leaving `deferred_task_id` NULL — and tusk-review-final-summary reads that column to distinguish "tasks created" from "skipped (duplicate)", so every deferred finding was silently counted as a dupe-skip. New end-to-end regression test (`tests/unit/test_review_defer_final_summary_flow.py`) drives the full defer → review-defer → review-final-summary pipeline against a real SQLite DB and asserts the created/skipped counts match the actual inserts.
...
- [TASK-114] Add `tusk review-diff-range <task_id>` wrapper and rewire `/review-commits` Steps 3 and 8 to call it instead of two copies of the same inline bash: the Step 3 "run `git diff <default>...HEAD`, check emptiness, fall back to `git log --grep=\[TASK-N\]` and rebuild the range from oldest^..newest" sequence, and the Step 8 "measure diff size differently depending on whether you're on the default branch (use the recovered TASK-N range from Step 3) or a feature branch (use `$(git merge-base HEAD origin/<default>)..HEAD`)" split; the helper returns `{range, diff_lines, summary, recovered_from_task_commits}` as JSON so Step 3 can pass `$DIFF_SUMMARY` straight to `tusk review start --diff-summary` (no re-parsing) and Step 8 can read `diff_lines` for the ~200-line inline-review threshold without caring which branch it's on, removing ~40 lines of duplicated prose and eliminating drift risk between the two copies; seven new unit tests in `tests/unit/test_review_diff_range.py` pin the primary `<default>...HEAD` range, the [TASK-N] commit-range recovery when primary is empty, the empty-diff error path, an exact-task-id grep boundary (so `[TASK-42]` must not pull in commits for `[TASK-421]`), and the CLI-layer JSON shape + invalid-task-id exit path

## [670] - 2026-04-19

- [TASK-116] Add `tusk review-final-summary <review_id>` wrapper and rewire `/review-commits` Step 10 to call it instead of `tusk review summary` + `tusk review-verdict` + a hand-maintained APPROVED/CHANGES_REMAINING → display-label table; the helper reads directly from `code_reviews` / `review_comments`, aggregates counts across all of a task's reviews including superseded passes (so the block reflects cumulative findings across multi-pass review journeys) while recomputing the verdict against non-superseded reviews only (lockstep with `tusk review verdict`), distinguishes "tasks created" from "skipped (duplicate)" via `review_comments.deferred_task_id` populated-vs-NULL, and owns the `APPROVED` / `CHANGES REMAINING` (space, not underscore) display-label mapping inside the wrapper so future verdict additions only need to change one place instead of every caller's bash prose; six new unit tests in `tests/unit/test_review_final_summary.py` pin the APPROVED path, the CHANGES_REMAINING → "CHANGES REMAINING" mapping, the superseded-review exclusion invariant for both resolved and stale-unresolved must_fix comments, and the missing-review error path

## [669] - 2026-04-19

- Harden the hidden `--_rexec-src` flag introduced in v668 — the flag is internal plumbing for the re-exec handoff and is only set by our own `os.execv` call, always to a `tempfile.mkdtemp(prefix="tusk-upgrade-")` subpath, but nothing previously validated the passed path so a manual or accidental invocation like `tusk upgrade --_rexec-src /Users/foo/repo` would have steered the finally-block `shutil.rmtree(tmpdir, ignore_errors=True)` at `/Users/foo` (ignore_errors mode skips permission denials silently but still removes user-owned files); `main()` now resolves both the passed path and `tempfile.gettempdir()` via `os.path.realpath` (handles `/tmp` → `/private/tmp` on macOS) and exits with `Error: --_rexec-src must be a subpath of <tmp_root>` before entering the try/finally if the path escapes the system tempdir; four new unit tests in `tests/unit/test_upgrade_rexec_guard.py` pin the guard (rejects outside-tempdir path, rejects /etc, rejects /, accepts a hypothetical tempdir subpath) so this safety measure can't silently regress
...
- [TASK-83] Wire skill-run lifecycle into /tusk, /chain, /retro
...
- [TASK-79] Add skill_runs.task_id for per-task skill-cost attribution
...
- [TASK-91] Migrate tusk-lint rules 14/15 from subprocess to direct DB access

## [589] - 2026-04-17

- [TASK-90] Migrate tusk-lint rules 5/9/10 from subprocess to direct DB access

## [588] - 2026-04-17

- [TASK-89] Scope tusk-lint rule 6 to tasks closed in the last 30 days so retroactive historical violations no longer hang `tusk commit` (fixes #480)
...
- [TASK-39] Extend prerequisite warning to tusk task-start
...
- [TASK-38] Fix block-sql-neq hook false positive when not-equal operator appears in double-quoted string arguments
...
- [TASK-14] Add tusk init-fetch-bootstrap CLI command for project lib bootstrap fetching
...
- [TASK-681] Investigate reviewer false positives for moved/renamed code

## [480] - 2026-03-18

- [TASK-682] tusk merge should give clearer error when task worked on main with no feature branch
...
- [TASK-664] Shorten /create-task boilerplate in skills/loop/SKILL.md
...
- [TASK-649] Add tusk init-scan-codebase
...
- [TASK-645] Add tusk review-verdict <task_id>
...
- [TASK-570] Flip tusk sync-skills precedence — internal overrides public
...
- Clarify review-commits SKILL.md: diff_content must be passed verbatim, not paraphrased
...
### Changed
- `/loop` skill: added convention note encouraging `/create-task` as the default for all task creation over raw `bin/tusk task-insert`
...
### Changed
- `/tusk` skill: added convention note at the top encouraging `/create-task` as the default for all task creation over raw `bin/tusk task-insert`
...
### Removed
- `skills/review-pr/` directory replaced by `skills/review-commits/`
