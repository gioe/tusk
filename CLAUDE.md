...
- **Agent-time hooks** (`.claude/hooks/<name>.sh`) — invoked by Claude Code's `PreToolUse` machinery before specific tool calls (typically `Bash`). Used to block sqlite shell-outs, inject task context into prompts, run advisory lint, etc. Claude-Code-only — Codex installs skip this directory.
