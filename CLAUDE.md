...
bin/tusk dev-sync [--dry-run]                     # source-repo only: copy bin/tusk + bin/tusk-*.py + UNDERSCORE_BIN_FILES + VERSION + pricing.json into .claude/bin/ and refresh tusk-lint.py.hash. Refuses outside the source repo.
...
- **`/cost`** — Report the recorded cost of a task, objective, or complete project
...
**Source-repo auto-refresh of `.claude/bin/` after `tusk merge` (issues #863 and #1237):** in the tusk source repo, `bin/` and root `pricing.json` are the sources of truth and `.claude/bin/` is a refresh-on-demand cache populated by `tusk dev-sync`. When `tusk merge` finishes in a primary checkout that has a `.claude/bin/` directory, it compares each `bin/tusk-*.py`, `bin/tusk`, and `pricing.json` against its deployed counterpart and, if any pair drifts, invokes `tusk dev-sync` automatically before returning. A single stderr line names the drifted files. Consumer installs (no `.claude/bin/` in the primary) are a silent no-op. Set `TUSK_NO_DEPLOYED_BIN_REFRESH=1` to disable. This closes the gap where source-repo code or pricing changes shipped via merge but did not take effect for subsequent same-session tasks because hooks and skill-run commands kept loading stale `.claude/bin/` copies.
